from config_agentic import CATEGORY_MODEL_NAME, FORCE_CATEGORY_CLASSIFICATION, CATEGORY_VALID_LABELS, RETRY_FAILED_CATEGORY, MONGO_COLLECTION, VERBOSE
...
PROMPT_TEMPLATE_PATH = SCRIPT_DIR / "prompts" / "prompt_category_template.txt"
...
def classify_category(email):
    prompt = load_prompt_template(PROMPT_TEMPLATE_PATH, {
        "valid_labels": ", ".join(CATEGORY_VALID_LABELS),
        "subject": email.get("subject", ""),
        "sender": email.get("sender", ""),
        "body": email.get("body", "")
    })
    return run_llm(prompt, model=CATEGORY_MODEL_NAME)

def main():
    
    # Build classification query
    if FORCE_CATEGORY_CLASSIFICATION:
        query = {}
    elif RETRY_FAILED_CATEGORY:
        query = {
            "$or": [
                {"category_classification": {"$exists": False}},
                {"category_classification.status": "Failed"}
            ]
        }
    else:
        query = {"category_classification": {"$exists": False}}
    ...
    print_progress(f"📂 Starting category classification on {total} emails...\n")

    for i, email in enumerate(emails, 1):

        result = classify_category(email)
